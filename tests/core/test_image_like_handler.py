...
class TestImageLikeHandler:
    def test_ngff_image(self, ome_zarr_image_v04_path: Path) -> None:
        ...
        image_handler_1 = ImageLike(store=store, path="1")
        assert image_handler_1._get_array(c=0, t=0, z=0, x=0, y=0) == 1

    def test_ngff_image_fs(self, ome_zarr_image_v04_fs: Path) -> None:
        from ngio.core.image_like_handler import ImageLike

        store = ome_zarr_image_v04_fs
        image_handler = ImageLike(store=store, path="0")
    ...
    def test_ngff_image_from_pixel_size(self, ome_zarr_image_v04_path: Path) -> None:
        from ngio.core.image_like_handler import ImageLike
