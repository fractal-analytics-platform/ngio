...
class TestImageLikeHandler:
    def test_ngff_image(self, ome_zarr_image_v04_path):
        ...
        assert image_handler.array(c=0).shape == shape[1:]
        assert (
            image_handler.array(c=0, preserve_dimensions=True).shape == (1,) + shape[1:]
        )

        image_handler.set_array(patch=np.ones((3, 10, 256, 256), dtype=np.uint16))
        assert image_handler.array(c=0, t=0, z=0, x=0, y=0) == 1

        image_handler.consolidate(order=0)

        image_handler_1 = ImageLike(store=ome_zarr_image_v04_path, path="1")
        assert image_handler_1.array(c=0, t=0, z=0, x=0, y=0) == 1
